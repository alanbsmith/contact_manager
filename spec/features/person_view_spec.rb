...
describe 'the person view', type: :feature do
  ...
  describe 'email addresses' do
    before(:each) do
      person.email_addresses.create(address: "sholmes@example.com", person_id: 1)
      person.email_addresses.create(address: "jwatson@example.com", person_id: 2)
      visit person_path(person)
    end
    ...
    it 'adds a new email address' do
      ...
      page.fill_in('email address', with: 'sholmes@example.com')
      page.click_button('Add')
      expect(current_path).to eq(person_path(person))
      expect(page).to have_content('sholmes@example.com')
    end
